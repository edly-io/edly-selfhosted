...
class OnboardingSurveyAPI(APIView):
    ...
    def get(self, request):
        ...
        if onboarding_survey and onboarding_survey.form_filled:
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if not onboarding_survey:
                onboarding_survey = OnboardingSurvey.objects.create(form_filled=form_filled)
            else:
                onboarding_survey.form_filled = form_filled
                onboarding_survey.save()
            return Response({"message": "Form status updated successfully."}, status=HTTP_200_OK)
